    ...
    var badgeValue: String? = "" {
        didSet{
            if (badgeValue?.isEmpty)!   || (badgeValue == "") || ((badgeValue == "0") && shouldHideBadgeAtZero) {
                ...
                self.badgeLabel  = UILabel(frame: CGRect(x: self.badgeOriginX , y: self.badgeOriginY, width: 5, height: 5))
    ...
    fileprivate  func updateBadgeFrame() {
        ...
        if self.badgeLabel != nil {
            if let customView = self.customView {
                ...
                self.badgeLabel?.frame = CGRect(x: self.badgeOriginX, y: self.badgeOriginY, width: 10, height: 10)
                self.badgeLabel?.layer.cornerRadius = 10 / 2
