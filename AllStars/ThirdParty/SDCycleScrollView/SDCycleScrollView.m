...
-(void)setTopTitlesGroup:(NSArray *)topTitlesGroup{
    _topTitlesGroup = topTitlesGroup;
    if (self.onlyDisplayText) {
        NSMutableArray *temp = [NSMutableArray new];
        for (int i = 0; i < _topTitlesGroup.count; i++) {
            [temp addObject:@""];
        }
        self.backgroundColor = [UIColor clearColor];
        self.imageURLStringsGroup = [temp copy];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:nil options:SDWebImageAllowInvalidSSLCertificates];
        } else {
    ...
    cell.imageView.layer.cornerRadius = self.cornerRadiusValue;
    cell.imageView.clipsToBounds =YES;
    if (_titlesGroup.count && itemIndex < _titlesGroup.count) {
        cell.title = _titlesGroup[itemIndex];
    }
    
    if (_topTitlesGroup.count && itemIndex < _topTitlesGroup.count) {
        cell.topTitle = _topTitlesGroup[itemIndex];
    }
